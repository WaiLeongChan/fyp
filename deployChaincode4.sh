...
# Modified by my own
CC_SRC_PATH="./artifacts/src/github.com/simple_sc_4/go"
CC_NAME="simple_sc_4"
...
queryInstalled() {
    setGlobalsForPeer0Org1
    peer lifecycle chaincode queryinstalled >&log4.txt
    cat log4.txt
    PACKAGE_ID=$(sed -n "/${CC_NAME}_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}" log4.txt)
...
chaincodeInvoke() {
    ...
    peer chaincode invoke -o localhost:7050 \
        ...
        --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_ORG1_CA \
        --peerAddresses localhost:15051 --tlsRootCertFiles $PEER0_ORG5_CA \
        -c '{"function": "createCar","Args":["{\"id\":\"7\",\"make\":\"Toyota\",\"addedAt\":1600135409912,\"model\":\"SUV\", \"color\":\"Grey\",\"owner\":\"jackie\"}"]}'
...
chaincodeQuery() {
    setGlobalsForPeer0Org1
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "GetCarById","Args":["7"]}'
}
